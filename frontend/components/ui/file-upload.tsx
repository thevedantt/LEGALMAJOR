  ...
  return (
    <div className="w-full" {...getRootProps()}>
      <motion.div
        onClick={handleClick}
        whileHover="animate"
         className="group/file relative block w-full cursor-pointer overflow-hidden rounded-lg p-10 bg-[#b3e5fc] border-2 border-dashed border-[#c1121f]"
      >
        <input
          ref={fileInputRef}
        ...
        <div className="absolute inset-0 [mask-image:radial-gradient(ellipse_at_center,white,transparent)]">
           <GridPattern colorPrimary="#b3e5fc" colorSecondary="#c1121f" />
        </div>
...
export function GridPattern({ colorPrimary = "#b3e5fc", colorSecondary = "#c1121f" }: { colorPrimary?: string, colorSecondary?: string }) {
  const columns = 41;
  const rows = 11;
  return (
    <div className="flex shrink-0 scale-105 flex-wrap items-center justify-center gap-x-px gap-y-px" style={{ backgroundColor: colorPrimary }}>
      {Array.from({ length: rows }).map((_, row) =>
        Array.from({ length: columns }).map((_, col) => {
          ...
          return (
            <div
              key={`${col}-${row}`}
              style={{ backgroundColor: index % 2 === 0 ? colorPrimary : "#e0f7fa", boxShadow: index % 2 !== 0 ? `0 0 1px 3px ${colorSecondary}22 inset` : undefined }}
